...
model_df <- df %>%
  ...
  filter(n() >= 3) %>%
  ungroup() %>%
  # Factorize on the FULL dataset BEFORE splitting so levels are
  # identical in train and test; predict() never sees a new level.
  mutate(
...
# Re-apply the full level set to both splits so no split is
# missing a level that the other split (and the model) knows about.
